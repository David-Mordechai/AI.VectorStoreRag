    ...
    [FromKeyedServices("AppShutdown")] CancellationTokenSource appShutdownCancellationTokenSource) : IHostedService
{
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        _dataLoaded = ragConfigOptions.Value.BuildCollection ? LoadDataAsync(cancellationToken) : Task.CompletedTask;

        // Start the chat loop.
        _ = ChatLoopAsync(cancellationToken);
    ...
    private async Task ChatLoopAsync(CancellationToken cancellationToken)
    {
        ...
        while (_dataLoaded is { IsCompleted: false } && !cancellationToken.IsCancellationRequested)
        {
            await Task.Delay(1_000, cancellationToken).ConfigureAwait(false);
        }

        // If data loading failed, don't start the chat loop.
        if (_dataLoaded is { IsFaulted: true })
        {
